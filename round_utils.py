...
def get_lyrics_sequence_by_difficulty(lyrics: List[Text], difficulty: DifficultyType) -> Text:
    ...
    random_sequence_start_index = random.randint(0, len(lyrics) - lyric_lines_sequence_length)

    return "\n".join(lyrics[random_sequence_start_index : random_sequence_start_index + lyric_lines_sequence_length:])
